...
def make_result():
    ...
    result.multi_az.resources = [ResourceScore("rds", "db1", "us-east-1", 100.0, "MultiAZ is enabled")]
    result.multi_az.service_scores = {"rds": 100.0}
    result.multi_az.account_score = 100.0
...
def test_report_shape_and_passthrough():
    ...
    assert acct["scores"] == {"multi_az": 100.0, "cross_region": None}
    dim = acct["dimensions"]["multi_az"]
    assert dim["service_scores"] == {"rds": 100.0}
